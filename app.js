...
app.post(
  ...
  async (req, res) => {
    try {
      var userID = req.body["decoded"].data._id;
      var { product } = req.body; // Chỉ lấy product._id, không cần variant_id
      ...
      // Cập nhật phần xóa sản phẩm theo ObjectId
      const deleteInCart = await users
        .findOneAndUpdate(
          { _id: new mongoose.Types.ObjectId(userID) },
          {
            $pull: {
              // $pull dùng xóa sản phẩm có product._id trong giỏ hàng
              cart: {
                product: new mongoose.Types.ObjectId(product),
              },
            },
      ...
      if (deleteInCart !== null) {
        ...
        return res.send(
          response(404, "", "Không có sản phẩm này trong giỏ hàng")
        );
      }
    } catch (error) {
      if (error.errorResponse)
